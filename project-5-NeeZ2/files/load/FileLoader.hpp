...
private:
    const static int JPEG_SAVE_QUALITY = 100;
    const static int NUM_CHANNELS = 3;
    const static std::string SAVE_IMAGE_AS_EXTENSION;

    void checkExists(const fs::path& path) const;
    void checkNotExists(const fs::path& path) const;
    void checkDirectory(const fs::path& path) const;
    void checkExtensionIs(const fs::path& path, const std::vector<std::string>& supportedExtensions) const;
    std::unique_ptr<unsigned char[]> toArray(const std::vector<std::vector<Pixel>>& pixels) const;
...
public:
     ...
     */
    TextFile loadTextFile(const std::string& str_path_to_file) const;
     ...
     */
    ImageFile loadImageFile(const std::string& str_path_to_file) const;

    /**
     * Saves an image to the specified directory in a JPEG format, with 100% quality if a file
     * with the name does not already exist in the target directory.
     *
     * @param directory A const reference to a STRING representing the directory
     * in which to save the image file.
     *
     * @throws std::invalid_argument if:
     *  1. The specified directory does not exist or is not a valid directory.
     *  2. The file already exists.
     *
     * @see stb_image_write.h Read the documentation section
     * @see FileLoader::toArray
     */
    void saveImageFile(const ImageFile& image, const std::string& file_stem, const std::string& directory = "./") const;
};