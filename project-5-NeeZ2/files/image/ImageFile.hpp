...
public:
     ...
     *
     * @pre For simplicity, we assume that the extension is that of an image file -- you need not validate it.
     *
     ...
     * 2. The pixel buffer is set to the provided vector, or initialized to the default size with all black pixels if omitted.
     */
    ImageFile(
    ...
    /**
     * Updates the pixel buffer of the image.
     *
     * @param pixels A const reference to a 2D vector of Pixel objects representing the new image data.
     * @post
     * 1. The pixels_ member variable is updated to match the provided 2D vector.
